...
const ReviewsTab = ({ Reviews }) => {
  ...
  const sortedReviews = [...filteredReviews].sort((a, b) => {
    if (activeSort === "Most Recent") {
      return a._id - b._id; // Assuming lower ID means more recent
    }
      ...
      <div>
        ...
        <div className="text-center mb-6 ">
          ...
          <div className="text-xl font-bold mb-1">
            5<span className="text-lg text-villa-text-light">/5</span>
          </div>
          ...
          <Drawer>
            ...
            <DrawerContent>
              <DrawerHeader>
                <DrawerTitle>All Reviews ({Reviews.length})</DrawerTitle>
              </DrawerHeader>
              <div className="px-4 pb-4 max-h-96 overflow-y-auto">
                <div className="space-y-4">
                  {Reviews.map((review) => (
                    <div
                      key={review._id}
                      className="bg-villa-grey/30 p-4 rounded-lg"
                    >
                      <div className="flex items-start space-x-3">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center justify-between mb-1">
                            ...
                            {review.isTopReview && (
                              <span className="text-xs bg-villa-green text-white px-2 py-1 rounded-full">
                                Top Review
                              </span>
                            )}
                          </div>
                          <p className="text-xs text-villa-text-light mb-2">
                          ...
                          <div className="flex items-center space-x-1 mb-2">
                            ...
                            <span className="text-xs text-villa-text-light">
                              /5
                            </span>
                          </div>
                          <div className="flex flex-wrap gap-2 mb-3">
                            {review.categories.map((category) => (
                              <span
                                key={category}
                                className="px-2 py-1 bg-villa-green/20 text-villa-green rounded text-xs"
                              >
                                ✓ {category}
                              </span>
                            ))}
                ...
                <div className="flex-1">
                  ...
                  <div className="flex flex-wrap gap-2 mb-3">
                    {review.categories.map((category) => (
                      <span
                        key={category}
                        className="px-2 py-1 bg-green-500/20 text-villa-green rounded text-xs"
                      >
                        ✓ {category}
                      </span>
                    ))}
