...
export default function CottageDetailsDrawer({
  isOpen,
  open,
  onClose,
  onOpenChange,
  ...
  const activeCottage = cottage || tent;
  ...
  const handleSelect = () => {
    if (onSelectCottage) onSelectCottage(activeCottage);
    else if (onBookNow) onBookNow(activeCottage);
    else handleClose();
  };
  ...
  const images =
    activeCottage.cottageimages && activeCottage.cottageimages.length > 0
      ? activeCottage.cottageimages
      : activeCottage.images && activeCottage.images.length > 0
      ? activeCottage.images
      : ["/placeholder.svg"];
  ...
  const weekdayPrice =
    activeCottage.pricing?.weekdayPrice || activeCottage.price || 0;
  const weekendPrice =
    activeCottage.pricing?.weekendPrice || weekdayPrice;
  const totalUnits =
    activeCottage.totalcottage ||
    activeCottage.totaltents ||
    activeCottage.totalCottages ||
    1;
        ...
        <div className="px-5 pb-3 border-b border-gray-100 flex items-center justify-between">
          <div>
            <div className="inline-flex items-center gap-1 text-[11px] font-semibold text-[#ff6900] uppercase tracking-wider">
              ...
              <span>Cottage Unit</span>
            </div>
            <DrawerTitle className="text-xl font-bold text-gray-900">
              {activeCottage.cottageType || activeCottage.tentType || activeCottage.name || "Cottage"}
            </DrawerTitle>
          </div>
          <button
            type="button"
        ...
        <ScrollArea data-vaul-no-drag className="max-h-[calc(88vh-140px)] px-5 py-4">
          ...
          <div className="relative w-full h-56 rounded-2xl overflow-hidden bg-gray-100 border border-gray-100">
            <Image
              ...
              alt={activeCottage.cottageType || "Cottage"}
              fill
              unoptimized
          ...
          <div className="grid grid-cols-3 gap-2">
            <div className="p-2.5 rounded-xl bg-neutral-50 border border-neutral-150 flex flex-col items-center text-center">
              ...
              <span className="text-xs font-bold text-gray-900">
                Up to {activeCottage.maxCapacity || 2} Guests
              </span>
            </div>
            <div className="p-2.5 rounded-xl bg-neutral-50 border border-neutral-150 flex flex-col items-center text-center">
          ...
          {/* Amenities */}
          {activeCottage.amenities && activeCottage.amenities.length > 0 && (
            <div className="space-y-2">
              <h4 className="text-xs font-bold text-gray-900 uppercase tracking-wider">
                Unit Amenities
              </h4>
              <div className="grid grid-cols-2 gap-2">
                {activeCottage.amenities.map((amenity, idx) => (
                  <div
                    key={idx}
