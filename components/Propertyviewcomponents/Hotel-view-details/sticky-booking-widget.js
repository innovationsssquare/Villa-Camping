    ...
    useEffect(() => {
      ...
      async function loadCoupons() {
        try {
          let res = null;
          if (hotel?._id) {
            res = await Getallcouponbypropertyid(hotel._id);
          }
          ...
          console.warn("[hotel widget] Error loading backend coupons:", err);
                ...
                <Popover>
                  ...
                  >
                    <CalendarComponent
                      ...
                      onSelect={(range) => {
                        if (range?.from) dispatch(setCheckin(range.from.toISOString()));
                        if (range?.to) dispatch(setCheckout(range.to.toISOString()));
                      }}
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-2">
                          ...
                          <span className="text-sm font-medium text-black transition-all duration-300">
                            {selectedGuest.adults} Adults, {selectedGuest.childrenn} Children
                          </span>
                        </div>
