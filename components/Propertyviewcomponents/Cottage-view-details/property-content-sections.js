...
import CottageDetailsDrawer from "@/components/Cottagescreen/CottageDetailsDrawer";
  ...
  return (
    <div className="w-full space-y-12">
      ...
      >
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {(cottage?.images?.slice(0, 4) || []).map((img, index) => {
              const titles = ["FULLY-SERVICED COTTAGES", "COZY GETAWAY", "PREMIUM INTERIORS", "CURATED EXPERIENCES"];
              return (
                <div
                  key={index}
        ...
        <Card className="border-gray-200 border transition-all duration-300">
          <CardContent className="p-6">
            <div className="space-y-6">
              {cottage?.cancellationPolicy?.length > 0 && (
              ...
              {cottage?.CampingRules?.length > 0 && (
                <div className="transition-all duration-300 hover:bg-gray-50 rounded-lg p-4 -m-4">
                  <h3 className="font-semibold text-lg mb-2 text-black transition-all duration-300">
                    Cottage Rules
                  </h3>
                  <ul className="space-y-2 text-gray-700 text-sm">
                    {cottage.CampingRules.map((rule, index) => (
                      <li key={index} className="transition-all duration-300 hover:text-black">
                        • {rule}
                      </li>
                    ))}
              ...
              {cottage?.paymentTerms?.length > 0 && (
                <div className="transition-all duration-300 hover:bg-gray-50 rounded-lg p-4 -m-4">
                  <h3 className="font-semibold text-lg mb-2 text-black transition-all duration-300">
                    Payment Terms
                  </h3>
                  <ul className="space-y-2 text-gray-700 text-sm">
                    {cottage.paymentTerms.map((term, index) => (
                      <li key={index} className="transition-all duration-300 hover:text-black">
                        • {term}
                      </li>
                    ))}
      ...
      >
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center space-x-2 transition-all duration-300 hover:transform hover:scale-105">
            ...
            <span className="text-gray-500 transition-all duration-300">
              • {cottage?.totalReviews || reviews.length} reviews
            </span>
          </div>
        </div>
        ...
        {(cottage?.totalReviews || reviews.length) > 3 && (
          <Button
            variant="outline"
          ...
          >
            Show all {cottage?.totalReviews || reviews.length} reviews
          </Button>
        )}
      </section>
            ...
            <div className="flex items-start space-x-4 mb-4">
              ...
              <div>
                <h3 className="font-semibold text-lg text-black transition-all duration-300">
                  {cottage?.address?.addressLine || "Address"}
                </h3>
                <p className="text-gray-600 transition-all duration-300">
                  {cottage?.address?.area}{cottage?.address?.city ? `, ${cottage.address.city}` : ""}
                </p>
              </div>
            </div>
            {cottage?.coordinates && (
              <GoogleMap
                coordinates={cottage.coordinates}
                zoom={14}
                className="w-full h-64 rounded-lg mb-4 border border-gray-300"
              />
            )}
            {cottage?.nearbyattractions?.length > 0 && (
              <div className="mt-4">
      ...
      >
        ...
        {cottage?.faqs?.length > 0 ? (
          <Accordion type="single" collapsible className="space-y-3">
            {cottage.faqs.map((item, index) => (
              <AccordionItem
                key={item._id || index}
                value={`faq-${index}`}
      ...
      {/* Cottage Details Overlay */}
      <CottageDetailsDrawer
        tent={selectedCottage}
        open={drawerOpen}
