...
export default function GoogleMap({
  ...
  if (error) {
    return (
      <div className={`${className} bg-gray-200 border border-gray-300 flex items-center justify-center`}>
        <div className="text-center p-4">
          <div className="text-gray-500 mb-2">⚠️</div>
          <p className="text-sm text-gray-600">{error}</p>
          <p className="text-xs text-gray-500 mt-2">Add your Google Maps API key to environment variables</p>
        </div>
      </div>
    )
  }
