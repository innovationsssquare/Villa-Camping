  ...
  return (
    <div className="w-full space-y-12">
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {(villa?.images?.slice(0, 4) || []).map((img, index) => {
              const titles = ["FULLY-SERVICED VILLAS", "PARTY VIBE", "PREMIUM INTERIORS", "CURATED EXPERIENCES"];
              return (
                <div
                  key={index}
        ...
        <Card className="border-gray-200 border transition-all duration-300">
          <CardContent className="p-6">
            <div className="space-y-6">
              {villa?.cancellationPolicy?.length > 0 && (
        ...
        {villa?.spaces?.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {villa.spaces.map((space, index) => (
              <Card key={space._id || index} className="overflow-hidden border-gray-200 transition-all duration-300 hover:shadow-lg">
                {space.image && (
                ...
                <CardContent className="p-4 space-y-3">
                  <h3 className="font-semibold text-lg text-black">{space.name}</h3>
                  {space.description && (
                  ...
                  {space.details?.length > 0 && (
                    <ul className="space-y-1">
                      {space.details.map((detail, i) => (
                        <li key={i} className="text-sm text-gray-600 transition-all duration-300 hover:text-black hover:translate-x-1">
                          • {detail}
                        </li>
                      ))}
                    </ul>
      ...
      >
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center space-x-2 transition-all duration-300 hover:transform hover:scale-105">
            ...
            <span className="text-xl font-semibold text-black transition-all duration-300">
              {villa?.averageRating || "N/A"}
            </span>
            <span className="text-gray-500 transition-all duration-300">
              • {villa?.totalReviews || reviews.length} reviews
            </span>
          </div>
        </div>
        ...
        {(villa?.totalReviews || reviews.length) > 3 && (
          <Button
            variant="outline"
          ...
          >
            Show all {villa?.totalReviews || reviews.length} reviews
          </Button>
        )}
      </section>
      ...
      >
        ...
        <Card className="border-gray-200 transition-all duration-300 hover:shadow-lg">
          <CardContent className="p-6">
            <div className="flex items-start space-x-4 mb-4">
              ...
              <div>
                <h3 className="font-semibold text-lg text-black transition-all duration-300">
                  {villa?.address?.addressLine || "Address"}
                </h3>
                <p className="text-gray-600 transition-all duration-300">
                  {villa?.address?.area}{villa?.address?.city ? `, ${villa.address.city}` : ""}
                </p>
              </div>
            </div>
            {villa?.coordinates && (
            ...
            {villa?.nearbyattractions?.length > 0 && (
              <div className="mt-4">
        ...
        {villa?.exploreStay?.length > 0 ? (
          <Accordion type="single" collapsible className="space-y-3">
            {villa.exploreStay.map((item, index) => (
              ...
              >
                <AccordionTrigger className="font-medium hover:no-underline text-black">
                  {item.title}
                </AccordionTrigger>
