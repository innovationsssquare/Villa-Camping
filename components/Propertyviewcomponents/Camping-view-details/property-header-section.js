...
export default function PropertyHeaderSection() {
  ...
  const remainingCount = Math.max(0, (allAmenities.length || 0) - 6);
    ...
    <div id="overview" className="bg-white rounded-3xl p-6 sm:p-8 border border-neutral-150 shadow-xs relative">
      ...
      <div className="mb-5">
        ...
        <div className="flex items-center gap-2 text-gray-600">
          ...
          <span className="text-sm font-medium text-gray-700">
            {camping?.address?.area ? `${camping.address.area}, ` : ""}{camping?.address?.city || "Pawana"}
          </span>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2.5 mb-6">
        ...
        >
          ...
          {totalTentsCount} Tents Available
        </Badge>
        ...
        {camping?.baths > 0 && (
          <Badge
            ...
            className="bg-neutral-100/90 text-gray-800 border border-neutral-200/60 px-4 py-2 rounded-full font-medium"
          >
            <Bath className="w-3.5 h-3.5 mr-2 text-[#ff6900]" />
            {camping.baths} Clean Washrooms
          </Badge>
        )}
