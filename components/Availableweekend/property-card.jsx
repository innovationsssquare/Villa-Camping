...
import { Heart, Star, MapPin, Users, Bed, Bath, ArrowRight, ChevronLeft, ChevronRight } from "lucide-react"
...
export function PropertyCard({ property }) {
  ...
  useEffect(() => {
    if (!isMobile || !isAutoPlaying || !property.images || property.images.length <= 1) {
      return
    }
  ...
  return (
    <Card isFooterBlurred className="w-full h-[400px] group hover:shadow-lg transition-all duration-300">
          ...
          <>
            ...
            <Button
              ...
              className={cn(
                ...
                currentImageIndex === property.images.length - 1 && "opacity-0 pointer-events-none",
              )}
              onPress={nextImage}
            >
      ...
      <CardFooter className="absolute bg-black/60 bottom-0 z-10 border-t-1 border-white/20 justify-between">
        <div className="flex flex-col gap-1 flex-grow">
          {/* Property Name */}
          <h3 className="text-white font-semibold text-lg leading-tight">{property.name}</h3>
          ...
          <div className="flex items-center gap-2 mt-1">
            <span className="text-lg font-bold text-white">₹{property.price.toLocaleString()}</span>
            {property.originalPrice && (
              <span className="text-xs text-white/60 line-through">₹{property.originalPrice.toLocaleString()}</span>
            )}
          </div>
