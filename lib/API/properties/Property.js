...
const resolveCategoryId = (id) => {
  if (!id) return id;
  const str = String(id).trim().toUpperCase();
  if (KNOWN_CATEGORY_IDS[str]) return KNOWN_CATEGORY_IDS[str];
  return id;
};
...
export const GetAllproperties = async ({
  ...
  try {
    ...
    if (categoryId) params.append("categoryId", resolveCategoryId(categoryId));
...
export const Getpropertybyid = async ({ categoryId, propertyId }) => {
  ...
  try {
    let result = await fetch(
      `${BaseUrl}/User/property/${resolveCategoryId(categoryId)}/${propertyId}`,
      {
        method: "GET",
        headers: {
          "Content-type": "application/json",
